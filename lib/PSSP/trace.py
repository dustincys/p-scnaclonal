...
class TraceDB(object):
    ...
    def __init__(self, out_dir, mutations):
        ...
        if not os.path.exists(os.path.dirname(os.path.abspath(out_dir))):
            raise Exception(
                "Folder {0} does not exist to create pyclone file in.".format(
    ...
    def _open_files(self, out_dir):
        ...
        '''
        mode = 'w'
        self._alpha_file = bz2.BZ2File(
            os.path.join(out_dir, 'alpha.tsv.bz2'), mode)
        self._frequencies_file = bz2.BZ2File(os.path.join(
            out_dir, 'cellular_frequencies.tsv.bz2'), mode)
        self._labels_file = bz2.BZ2File(
            os.path.join(out_dir, 'labels.tsv.bz2'), mode)
        self._phi_file = bz2.BZ2File(os.path.join(out_dir, 'phi.tsv.bz2'), mode)
        self._alpha_writer = csv.writer(self._alpha_file, delimiter='\t')
        self._frequencies_writer = csv.writer(
            self._frequencies_file, delimiter='\t')
